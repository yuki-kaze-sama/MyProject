...
function deleteBill(obj){
	$.ajax({
		type:"GET",
		url:path+"/billdelete",
		data:{method:"delbill",billcode:obj.attr("billcc")},
		dataType:"json",
...
$(function(){
	$(".viewBill").on("click",function(){
		...
		var obj = $(this);
		window.location.href=path+"/billview?method=view&billcc="+ obj.attr("billcc");
	});
	
	$(".modifyBill").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/tobillmodify?method=modify&billcc="+ obj.attr("billcc");
	});
	$('#no').click(function () {
		cancleBtn();
	});
